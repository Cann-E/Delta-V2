...
def generate_pdf_for_request(request):
    ...
    logger.debug(
        f"User Info: First Name: {request.user.first_name}, Last Name: {request.user.last_name}, UH ID: {getattr(request.user, 'uh_id', 'Not set')}")
    ...
    placeholders = {
        ...
        "MAILING_ADDRESS": str(
            getattr(request.user, "mailing_address", "123 University St.") or "123 University St.").strip(),
